...
def generate_brand_wise_report(input_file, writer):
    """
    Generates the Brand Wise Report and writes it to the output Excel file.
    ...
    unmatched_codes = merged_data[merged_data["Brand"].isna()]
    if not unmatched_codes.empty:
        print(f"Warning: {len(unmatched_codes)} product codes in 'movements' were not found in 'products'.")

    # Pivot the data to get the Brand Wise Report
    try:
        print("Creating the pivot table for the Brand Wise Report...")
    ...
    brand_wise_report = pivot_table.reset_index()

    # Write the report to the Excel file
    try:
        print("Writing the Brand Wise Report to the Excel sheet...")
        brand_wise_report.to_excel(writer, sheet_name="Brand Wise", index=False, startrow=1)
    except Exception as e:
    ...
    try:
        ...
        worksheet = writer.sheets["Brand Wise"]

        # Add a descriptive title
        worksheet.write(0, 0, "Brand Wise Report")
    ...
    print("Brand Wise Report generation complete.")

if __name__ == "__main__":
    try:
        with pd.ExcelWriter("output_file.xlsx", engine="xlsxwriter") as writer:
            print("Generating the Brand Wise Report...")
            generate_brand_wise_report("input_file.xlsx", writer)
    except Exception as e:
        print(f"An error occurred in brand wise report: {e}")
